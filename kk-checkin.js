...
async function openKkCheckin() {
  ...
  if (characters.length === 0) {
    listEl.innerHTML =
      '<p style="text-align:center; color: var(--text-secondary);">No characters available to spy on</p>';
  } else {
...
async function openKkHouseView(charId) {
  ...
  if (!chat.houseData) {
    ...
    const includeComputer = await showCustomConfirm(
      "Generation Options",
      "Generate computer contents?\n(Includes browser history, private files, Steam games, etc.)",
      { confirmText: "Yes, generate", cancelText: "No thanks" },
    );
...
async function generateHouseData(charId, includeComputer = true) {
  // 默认为true兼容旧代码
  const chat = state.chats[charId];
  showGenerationOverlay("Searching...");

  try {
    const { proxyUrl, apiKey, model } = state.apiConfig;
    if (!proxyUrl || !apiKey || !model) throw new Error("API not configured");
