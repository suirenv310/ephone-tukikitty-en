document.addEventListener("DOMContentLoaded", () => {
  let currentFilterContext = { type: "global", id: null }; // tracks which page's filter is currently open
  let activeGroupId = null; // tracks the currently open group ID
  let activeForumPostId = null; // tracks the currently open post ID
  let editingGroupId = null; // tracks the group ID being edited
  // ▼▼▼ Use this block to completely replace the old initializeDefaultGroups function ▼▼▼
  let activeForumFilters = {
    global: [], // for filtering the main group list
    group: {}, // for storing per-group post filters, e.g., { 1: ['Sci-Fi'], 2: ['Plot'] }
  };
  let isSelectionMode = false;
  let weiboHotSearchCache = [];
  let activeSeriesId = null; // currently viewed series ID
  let postReturnContext = "group"; // where to return from post detail
  const ongoingSeriesTasks = new Set(); // prevent duplicate series updates
  /**
   * [New] Get a random element from an array
   * @param {Array} arr - target array
   * @returns {*} - a random element from the array
   */
  function getRandomItem(arr) {
    // Safety check: if array is empty or doesn't exist, return empty string
    if (!arr || arr.length === 0) return "";
    // Return the element at a random index
  ...
  function resetCreatePostModal() {
    ...
    // [Core fix] We no longer simulate a click; instead we directly and safely set the state
  ...
  /**
   * [V3 Final Perfect Version] Render the forum main screen
   * Logic: built-in groups show SVG, user groups show custom images
   */
    ...
    if (groupsToRender.length === 0) {
      const message =
        globalFilters.length > 0
          ? "No groups found matching the filter criteria"
          : "No groups yet, click the \"+\" in the top right to create one!";
      listEl.innerHTML = `<p style="text-align:center; color: #8a8a8a; padding: 50px 0;">${message}</p>`;
      return;
    }

    // --- Core: icon generator (fixed: custom images take priority) ---
    const renderGroupIcon = (group) => {
      const name = group.name;
      const iconInput = group.icon || ""; // could be an emoji or a URL

      // 1. [Highest priority] Check if it is an image URL (starts with http or data:)
      // If the user provided a link, force-display the image regardless of its name!
      if (iconInput.startsWith("http") || iconInput.startsWith("data:")) {
        return `<img src="${iconInput}" class="forum-group-custom-img">`;
      }

      // 2. If no image URL, check if it is a [built-in group] and use a styled SVG
      const svgStyle = `width="24" height="24" fill="currentColor" viewBox="0 0 24 24"`;

      if (name.includes("Entertainment") || name.includes("Gossip")) {
      ...
      if (name.includes("Paranormal") || name.includes("Ghost")) {
      ...
      if (name.includes("Fanfic") || name.includes("Writing") || name.includes("Story")) {
      ...
      if (name.includes("Help") || name.includes("Vote") || name.includes("Assist")) {
      ...
      // 3. If neither a URL nor a built-in style match, check if it is an Emoji
      if (iconInput) {
        // Simple handling: display the input character directly as the icon (for Emoji)
    ...
    groupsToRender.forEach((group) => {
      ...
      item.innerHTML = `
            ...
            <div class="forum-group-info">
                ...
                <div class="forum-group-desc">${group.description || "No description"}</div>
                ${categoriesHtml}
            </div>
  ...
  /**
   * [New] Show action menu when long-pressing a group (edit or delete)
  ...
  async function showGroupActions(groupId, groupName) {
    // Call the existing modal function to show two options
    const choice = await showChoiceModal(`Manage Group "${groupName}"`, [
      { text: "✏️ Edit Group Info", value: "edit" },
      { text: "🗑️ Delete Group", value: "delete" },
    ]);

    // Execute different actions based on the user's choice
    if (choice === "edit") {
      // If the user chose "Edit", call the original edit function
      openGroupEditor(groupId);
    } else if (choice === "delete") {
      // If the user chose "Delete", call the original delete function
  ...
  async function openGroup(groupId, groupName) {
    ...
    // Show or hide specific UI based on group name
    if (groupName === "Fanfic Group") {
      fanficBar.style.display = "block";
      await populateFanficSelectors();
      await loadFanficPresets(); // ★ New: load presets
  ...
  /**
   * [New] Add a newly created post element to the top of the list
   * @param {object} post - the complete post object including ID
   */
  function prependNewPostElement(post) {
    const listEl = document.getElementById("group-post-list");

    // Check if the list is currently showing an "empty" message; if so, clear it
    const emptyMessage = listEl.querySelector("p");
    if (
      emptyMessage &&
      (emptyMessage.textContent.includes("no posts yet") ||
        emptyMessage.textContent.includes("No posts found"))
    ) {
      listEl.innerHTML = "";
    }

    // Create DOM element for the new post (this code is almost identical to the logic in renderGroupPosts)
    const commentCount = 0; // New posts always have 0 comments
    const item = document.createElement("div");
    ...
    item.innerHTML = `
      ...
      <button class="forum-post-delete-btn" title="Delete Post">×</button>
  `;

    // Use prepend() to add the new post to the [very front] of the list
    listEl.prepend(item);
  }
  ...
  /**
   * [Fixed] Render the post list and categories within a group (filtering supported)
   */
  async function renderGroupPosts(groupId) {
    ...
    for (const post of postsToRender) {
      // ★★★★★ This is the one and only core fix! ★★★★★
      // Before querying with post.id, use parseInt() to ensure it is always a number type.
      ...
      if (
        ...
        !categoriesForDisplay.includes("Long")
      ) {
        categoriesForDisplay.unshift("Long");
      } else if (
        post.lengthType === "short" &&
        !categoriesForDisplay.includes("Short")
      ) {
        categoriesForDisplay.unshift("Short");
      }
      if (post.chapterIndex) {
        categoriesForDisplay.unshift(`Chapter ${post.chapterIndex}`);
      }
  ...
  /**
   * [Key fix] Open a post and display its details and comments
   */
  ...
  /**
   * [Enhanced] Render post details and comments (updated: integrated with global passerby avatar library)
   */
  async function renderPostDetails(postId) {
    ...
    // Priority 1: if it is the user themselves
