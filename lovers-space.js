let activeLoversSpaceCharId = null; // Character ID corresponding to the current lovers' space
let activeLoveLetter = null; // The love letter object currently being viewed or replied to
let activeQuestionId = null; // The ID of the question currently being answered
let currentDiaryDate = null; // The date of the diary currently being edited or viewed
let tempUploadedPhotos = []; // Temporarily stored photo data pending upload
let lsActivityTimer = null; // Today's footprint timer ID
let pomodoroCustomPlaylist = [];
// Lovers' space music player state manager
let lsMusicState = {
  playlist: [], // Playlist
  currentIndex: -1, // Index of the currently playing song
...
// Configure looping playback logic (auto-play next track)
pomodoroState.bgmAudio.addEventListener("ended", () => {
  playNextPomoMusic();
});
// Listen for playback state changes to update the icon
...
/**
 * Check if an audio URL is playable
 * @param {string} url - Audio URL
 * @returns {Promise<boolean>} Whether the audio can be played
 */
...
/**
 * Fetch lyrics for an online song
 * @param {string} songId - Song ID
 * @param {string} source - Music platform source (netease/tencent)
 ...
 * @param {string} lrcContent - LRC lyrics content
 * @returns {Array} Parsed lyrics array [{time, text}]
 */
...
/**
 * Update music playback progress bar
 */
...
async function searchNeteaseMusic(name, singer) {
  try {
    ...
    await showCustomAlert(
      "NetEase Cloud direct connection failed",
      `If the browser console (F12) shows a CORS error, it means this API does not allow direct access. Error: ${e.message}`,
    );
    return [];
  }
}
...
/**
 * [New] Open the advanced share detail modal (cute style - with avatar)
 */
function openAdvancedShareDetail(shareData) {
  ...
  // 1. Get the current character's avatar
  // activeLoversSpaceCharId is a global variable that tracks which lovers' space is currently being viewed
  ...
  // Config data (defines icons, text, and color themes)
  const configMap = {
    movie: { icon: "🎬", label: "Movie Recommendation", class: "theme-movie" },
    book: { icon: "📖", label: "Book Share", class: "theme-book" },
    game: { icon: "🎮", label: "Game Recommendation", class: "theme-game" },
  };

  // If type is unknown, default to movie style
  ...
  titleEl.textContent = shareData.title || "Unknown Title";
  tagEl.textContent = config.label;

  // If there is author/singer info, append it after the title
  if (shareData.artist) {
    titleEl.textContent += ` - ${shareData.artist}`;
  }

  // Summary content
  summaryEl.textContent = shareData.summary || "(Very mysterious, no description written...)";

  // Thoughts content - "Char says..."
  thoughtsEl.textContent =
    shareData.thoughts || `Recommending "${shareData.title}" to you!`;
...
/**
 * Show a choice action modal
 * @param {string} title - Modal title
 * @param {Array} options - Options array [{text, value}]
 * @returns {Promise<string|null>} The value selected by the user, or null
 */
...
/**
 * Open the lovers' space entry - decide to enter directly or show character selector based on number of characters
 */
async function openLoversSpaceEntry() {
  ...
  if (singleChats.length === 0) {
    alert("You don't have any characters to create a lovers' space with yet. Create one first!");
    return;
  }
...
/**
 * Open the lovers' space character selector - shows character list and activation status
 */
async function openCharSelectorForLoversSpace() {
  ...
  singleChats.forEach((chat) => {
    ...
    const statusText = isLoversSpaceActive
      ...
      : '<span style="color: #8a8a8a;">Not Activated</span>';
...
/**
 * Open the lovers' space for the specified character
...
/**
 * Update the "days together" counter display in the lovers' space
...
function updateLoversSpaceDaysCounter(chat) {
  ...
  if (startDateString) {
    ...
    counterEl.textContent = `We have been together for ${diffDays} days`;
  } else {
    counterEl.innerHTML = `<a>Click "Settings" in the top right to record your first day together</a>`;
  }
}

/**
 * Render the lovers' space interface
 * @param {object} chat - Chat object
 */
async function renderLoversSpace(chat) {
  ...
  document.getElementById("ls-shares-list").innerHTML =
    '<p class="ls-empty-placeholder">No shares yet~</p>';
}

/**
 * Switch the lovers' space tab
...
/**
 * Render today's footprint page - defaults to the current day
 * @param {object} chat - Chat object
 */
...
/**
 * Render the daily footprint interface for a specified date
 * @param {object} chat - Chat object
...
function renderLSDailyActivityForDate(chat, date) {
  ...
  if (!chat || !chat.loversSpaceData) {
    viewEl.innerHTML =
      '<p class="ls-empty-placeholder">Data error, unable to load footprint.</p>';
    return;
  }
  ...
  let dateDisplay = `${date.getFullYear()}/${
    date.getMonth() + 1
  }/${date.getDate()}`;
  if (dateStr === todayStr) {
    dateDisplay += " (Today)";
    ...
    if (dateStr === todayStr) {
      contentContainer.innerHTML = `
        <div class="ls-activity-generate-container">
            <p>Today's footprint is still blank...</p>
            <button id="ls-generate-activity-btn">Generate Today's Footprint</button>
            <p class="hint">(This action can only be performed once per day)</p>
        </div>
    `;
      contentContainer.querySelector("#ls-generate-activity-btn").onclick =
        () => handleGenerateDailyActivity(chat);
    } else {
      contentContainer.innerHTML = `<p class="ls-empty-placeholder" style="margin-top: 50px;">No footprint was left on this day~</p>`;
    }
  } else {
...
/**
 * Open the daily footprint calendar modal
 * Initializes and displays the footprint calendar, supports month switching and date selection
 */
