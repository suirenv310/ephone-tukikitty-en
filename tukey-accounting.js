...
async function saveTukeyUserSettings() {
  ...
  tukeyUserSettings.username =
    document.getElementById("tukey-user-name-input").value.trim() || "Me";
...
async function handleAddCustomCategory(type) {
  const typeName = type === "expense" ? "Expense" : "Income";

  // 1. 输入名称
  const name = await showCustomPrompt(
    `Add ${typeName} Category`,
    "Enter category name (e.g. Milk Tea, Gaming)",
  );
  if (!name || !name.trim()) return;

  // 2. 选择图标来源
  const choice = await showChoiceModal("Select Icon", [
    { text: "📁 Upload from local", value: "local" },
    { text: "🌐 Use web URL", value: "url" },
    { text: "🎲 Use default icon", value: "default" },
  ]);
  ...
  if (choice === "local") {
    ...
    iconUrl = await showCustomPrompt("Icon URL", "Enter image link", "", "url");
  } else {
    // 默认图标
  ...
  if (!iconUrl) {
    alert("Invalid icon, adding cancelled.");
    return;
  }
...
async function handleAddCustomAccountType(categoryName) {
  // 1. 输入名称
  const name = await showCustomPrompt(
    `Add ${categoryName} Type`,
    "Enter type name (e.g. CMB, JD Finance)",
  );
  if (!name || !name.trim()) return;
  ...
  if (ACCOUNT_STRUCTURE[categoryName]) {
    ...
    alert(`Account type "${name.trim()}" added successfully!`);
  }
}
...
function openAccountTypeSelector() {
  ...
  titleEl.textContent = "Select Account Type";
...
async function openTukeyExportModal() {
  ...
  // 添加“所有账户”选项
  listEl.innerHTML += `<label><input type="checkbox" value="all" id="tukey-export-select-all"> <strong>All Accounts</strong></label>`;
...
async function exportTukeyReportToExcel() {
  ...
  await showCustomAlert("Please wait...", "Generating Excel file...");
  ...
  try {
    ...
    if (records.length === 0) {
      await showCustomAlert("No Data", "The selected accounts have no accounting records to export.");
      return;
    }

    // 2. 将数据格式化为适合Excel的数组
    const dataForSheet = records.map((rec) => ({
      Date: new Date(rec.timestamp).toLocaleString("zh-CN", {
        year: "numeric",
        month: "2-digit",
        day: "2-digit",
        hour: "2-digit",
      ...
      Type: rec.type === "expense" ? "Expense" : "Income",
    ...
    XLSX.writeFile(workbook, `TukeyAccounting-Report-${dateStr}.xlsx`);
  } catch (error) {
...
async function initTukeyAccounting() {
  ...
  document
    ...
    .addEventListener("click", async () => {
      ...
      const choice = await showChoiceModal("Accounting Group Settings", [
        { text: "👥 Manage Members (Add/Remove)", value: "manage_members" },
        { text: "🤖 AI Reply Settings", value: "ai_settings" },
        { text: "⚠️ Dissolve Group", value: "dissolve_group" },
      ]);
    ...
    if (rerollBtn) {
      ...
      if (!isNaN(recordId)) {
        ...
        if (typeof window.handleTukeyReroll === "function") {
          ...
          alert("Feature not ready, please refresh the page and try again.");
        }
      }
    }
  });
...
async function dissolveTukeyGroup() {
  ...
  const confirmed = await showCustomConfirm(
    "Confirm Dissolve",
    "Are you sure you want to dissolve the accounting group?\n\nNote:\n1. Previous accounting records will **not** be deleted and can still be viewed in reports.\n2. You can create a new accounting group afterwards.",
    { confirmButtonClass: "btn-danger" },
  );
  ...
  if (confirmed) {
    try {
      ...
      alert("Accounting group dissolved.");
    } catch (error) {
      console.error("解散群聊失败:", error);
      alert("Dissolve failed, please try again.");
    }
  }
}
...
async function openRecordEditor() {
  ...
  if (accounts.length === 0) {
    alert("Please add an account in the \"Wallet\" page first!");
    return;
  }
  accounts.forEach((acc) => {
...
async function saveTukeyRecordFromCard() {
  ...
  if (tukeyUserSettings.syncWithTaobao) {
    ...
    const syncDescription = `[TukeyAccounting Sync] ${newRecord.category} - ${newRecord.remarks || "No remarks"}`;
    await updateUserBalanceAndLogTransaction(syncAmount, syncDescription);
    console.log(
      `Taobao balance synced: ${syncAmount > 0 ? "+" : ""}${syncAmount.toFixed(2)}`,
    );
  }
  // ▲▲▲ 新代码粘贴结束 ▲▲▲
